...
def gear():
    [gl, jp, lore] = loadFiles(
        ['MasterParam.json', 'MasterParamJP.json', 'unit.json'])
    loc = Translation()
    gears = gl['Artifact']
    ...
    for g in gears:
        ...
        if 'abils' in g:
            for a in g['abils']:
                ...
                locA = loc[ability['skl1']] if ability['skl1'] in loc else {
                    'NAME': cjp[ability['skl1']]['name'], 'EXPR': cjp[ability['skl1']]['name']}
                locA.update({'restriction': restriction,
                             'iname': ability['skl1']})
        ...
        for r in range(g['rini']+1, g['rmax']+2):
            if ('attack'+str(r)) in g:
                ...
                c['atk_buff'][str(r)] = ""
                if 's_buff' in skill:
                    c['atk_buff'][str(r)] += 'Self: ' + \
                        buff(cmaster[skill['s_buff']], (r+1)*5, 30)
                elif 't_buff' in skill:
                    c['atk_buff'][str(r)] += 'Target: ' + \
                        buff(cmaster[skill['t_buff']], (r+1)*5, 30)
                elif 's_cond' in skill:
                    c['atk_buff'][str(r)] += 'Self: ' + \
                        condition(cmaster[skill['s_cond']], (r+1)*5, 30)
                elif 't_cond' in skill:
                    c['atk_buff'][str(r)] += 'Target: ' + \
                        condition(cmaster[skill['t_cond']], (r+1)*5, 30)
                else:
                    print(skill)
