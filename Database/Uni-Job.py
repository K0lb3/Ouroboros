...
def main():
    ...
    for raw,con in stats.items():
        unit['stats'][con] = int(unit[raw] + int((unit['m'+raw] - unit[raw]) / 99 * 1e5) * ((unitLevel - 1) / 1e5))
 ...
 # add jobs
    ...
    if 'jobsets' in unit:
        for i in unit['jobsets']:
            ...
            jobs.append({
                ...
                'jm': buff(mainc[mainc[job['master']]['t_buff']],1,2,True),
                'modifiers': {
                    'HP':   mod["hp"],
                    'Max Jewels': mod["mp"],
    ...
    for i in range(0,3):
        ...
        for key,stat in unit['stats'].items():
            try:
                j_stats[key]=jobs[i]['stats'][key]+int((stat)*(jobs[i]['modifiers'][key]+100)/100)
            except:
                pass

        for key,val in total_jm.items():
            j_stats[key]=int(j_stats[key]*(1+val))
        print(j_stats)
main()