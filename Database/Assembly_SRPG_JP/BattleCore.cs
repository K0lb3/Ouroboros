...
namespace SRPG
{
  public class BattleCore
  {
            ...
            while (enumerator.MoveNext())
            {
              ...
              if (current.Side == EUnitSide.Enemy)
              {
                ...
                if (instance.IsVSCpuBattle)
                  current.SetUnitFlag(EUnitFlag.ForceAuto, true);
              }
            }
          }
    ...
    private void BeginBattlePassiveSkill()
    {
      ...
      for (int index1 = 0; index1 < this.mAllUnits.Count; ++index1)
      {
        ...
        if (mAllUnit != null && !mAllUnit.IsDead && !mAllUnit.IsUnitFlag(EUnitFlag.OtherTeam))
        {
          ...
          if (conceptCard != null)
          {
            using (List<SkillData>.Enumerator enumerator = conceptCard.GetEnableCardSkills(mAllUnit.UnitData).GetEnumerator())
            {
              while (enumerator.MoveNext())
              {
                SkillData current = enumerator.Current;
                List<BuffEffect> cardSkillAddBuffs = conceptCard.GetEnableCardSkillAddBuffs(mAllUnit.UnitData, current.SkillParam);
                this.InternalBattlePassiveSkill(mAllUnit, current, true, cardSkillAddBuffs.ToArray());
              }
            }
          }
        }
    ...
    private void BeginBattlePassiveSkill(Unit unit)
    {
      ...
      if (conceptCard != null)
      {
        using (List<SkillData>.Enumerator enumerator = conceptCard.GetEnableCardSkills(unit.UnitData).GetEnumerator())
        {
          while (enumerator.MoveNext())
          {
            SkillData current = enumerator.Current;
            if (!current.IsSubActuate())
            {
              List<BuffEffect> cardSkillAddBuffs = conceptCard.GetEnableCardSkillAddBuffs(unit.UnitData, current.SkillParam);
              this.InternalBattlePassiveSkill(unit, current, true, cardSkillAddBuffs.ToArray());
            }
          }
        }
      }
    ...
    {
      if (target == null || target.IsGimmick && !target.IsBreakObj || (target.IsUnitFlag(EUnitFlag.OtherTeam) || self.IsSub && !skill.IsSubActuate()) || (!this.CheckSkillTarget(self, target, skill) || !is_duplicate && target.ContainsSkillAttachment(skill)))
        return;
...
label_4:
          ...
          if (skill.Target != ESkillTarget.Self || skill.Condition != ESkillCondition.None || flag)
            this.BuffSkill(ESkillTiming.Passive, self, target, skill, true, (LogSkill) null, SkillEffectTargets.Target, is_duplicate, buff_effects);
      ...
      for (int index = 0; index < strArray.Length; ++index)
      {
        using (List<TrickData>.Enumerator enumerator = effectAll.GetEnumerator())
        {
          while (enumerator.MoveNext())
          {
            ...
            if (strArray[index] == current.TrickParam.Name && !results.Contains(current))
              results.Add(current);
          }
        }
      }
              ...
              switch (mGimmickEvent.ev_type)
              {
                ...
                case eGimmickEventType.UNIT_KILL:
                  for (int index2 = 0; index2 < mGimmickEvent.targets.Count; ++index2)
                  {
                    ...
                    {
                      target.KeepHp = target.CurrentStatus.param.hp;
    ...
    public bool IsBonusObjectiveComplete(QuestBonusObjective bonus, ref int takeoverProgress)
    {
      switch (bonus.Type)
      {
        ...
        case EMissionType.LimitedTurn:
          takeoverProgress = this.mActionCount;
          int result1;
        ...
        case EMissionType.NoMercenary:
          ...
          if (flag1 && this.IsOrdeal)
          {
            using (List<Unit>.Enumerator enumerator = this.Units.GetEnumerator())
            {
              while (enumerator.MoveNext())
              {
                if (enumerator.Current.IsUnitFlag(EUnitFlag.IsHelp))
                {
                  flag1 = false;
                  break;
                }
        ...
        case EMissionType.TargetKillstreak:
          ...
          int num2;
          if (strArray1.Length >= 2 && this.mMaxTargetKillstreakDict.TryGetValue(strArray1[0].Trim(), out num2))
          {
            takeoverProgress = num2;
            int result14;
            if (int.TryParse(strArray1[1], out result14))
    ...
    private bool IsTeamPartySizeMax(int max_num, bool is_inc_mercenary)
    {
      ...
      for (int index = 0; index < this.mMaxTeamId; ++index)
      {
        ...
        using (List<Unit>.Enumerator enumerator = this.Units.GetEnumerator())
        {
          ...
          {
            Unit current = enumerator.Current;
            if (current.Side == EUnitSide.Player && current.IsPartyMember && current.TeamId == index && (!current.IsUnitFlag(EUnitFlag.IsHelp) || is_inc_mercenary))
              ++num;
          }
        }
        if (num > max_num)
    ...
    private bool IsMissionClearOnlyHeroConditions(bool check_main_member_only = false)
    {
      for (int index = 0; index < this.Units.Count; ++index)
      {
        if (this.Units[index].Side == EUnitSide.Player && this.Units[index].IsPartyMember && (!this.Units[index].UnitParam.IsHero() || check_main_member_only && (this.Units[index] == this.Friend || !this.mStartingMembers.Contains(this.Units[index]))))
          return false;
      }
      return true;
    }
    ...
    private int GetSkillEffectValue(Unit self, Unit target, SkillData skill, LogSkill log = null)
    {
      ...
      if (effectDeadRate != 0)
      {
        for (int index = 0; index < this.mUnits.Count; ++index)
        {
          if (!this.mUnits[index].IsUnitFlag(EUnitFlag.OtherTeam) && this.mUnits[index].IsDead && (!this.mUnits[index].IsGimmick && !this.CheckEnemySide(self, this.mUnits[index])))
            num6 += effectDeadRate;
        }
      }
      int num7 = 0;
    ...
    private void BuffSkill(ESkillTiming timing, Unit self, Unit target, SkillData skill, bool is_passive = false, LogSkill log = null, SkillEffectTargets buff_target = SkillEffectTargets.Target, bool is_duplicate = false, BuffEffect[] add_buff_effects = null)
    {
      ...
      bool flag4 = buffEffect.CheckBuffCalcType(BuffTypes.Buff, SkillParamCalcTypes.Add, false);
      bool flag5 = buffEffect.CheckBuffCalcType(BuffTypes.Buff, SkillParamCalcTypes.Add, true);
      bool flag6 = buffEffect.CheckBuffCalcType(BuffTypes.Buff, SkillParamCalcTypes.Scale);
      bool flag7 = buffEffect.CheckBuffCalcType(BuffTypes.Debuff, SkillParamCalcTypes.Add, false);
      bool flag8 = buffEffect.CheckBuffCalcType(BuffTypes.Debuff, SkillParamCalcTypes.Add, true);
      bool flag9 = buffEffect.CheckBuffCalcType(BuffTypes.Debuff, SkillParamCalcTypes.Scale);
      if (aag != eAbsorbAndGive.None)
      {
      ...
      if (skill.Condition == ESkillCondition.CardSkill && add_buff_effects != null)
      {
        ...
        for (int index = 0; index < add_buff_effects.Length; ++index)
        {
          if (add_buff_effects[index] != null)
          {
            ...
            flag4 |= add_buff_effects[index].CheckBuffCalcType(BuffTypes.Buff, SkillParamCalcTypes.Add, false);
            flag5 |= add_buff_effects[index].CheckBuffCalcType(BuffTypes.Buff, SkillParamCalcTypes.Add, true);
            flag6 |= add_buff_effects[index].CheckBuffCalcType(BuffTypes.Buff, SkillParamCalcTypes.Scale);
            flag7 |= add_buff_effects[index].CheckBuffCalcType(BuffTypes.Debuff, SkillParamCalcTypes.Add, false);
            flag8 |= add_buff_effects[index].CheckBuffCalcType(BuffTypes.Debuff, SkillParamCalcTypes.Add, true);
            flag9 |= add_buff_effects[index].CheckBuffCalcType(BuffTypes.Debuff, SkillParamCalcTypes.Scale);
    ...
    public bool UseSkill(Unit self, int gx, int gy, SkillData skill, bool bUnitLockTarget = false, int ux = 0, int uy = 0, bool is_call_auto_skill = false)
    {
      ...
      {
        bool flag2 = skill.IsCastSkill();
        if (!is_call_auto_skill)
        {
          BattleCore.OrderData currentOrderData = this.CurrentOrderData;
          if (currentOrderData != null)
            flag2 = currentOrderData.IsCastSkill;
        }
        if (flag2)
        {
        ...
        else
        {
          ...
          if (self == this.CurrentUnit)
            flag3 = self.IsDead || (!self.IsUnitFlag(EUnitFlag.Moved) ? !self.IsEnableMoveCondition(false) && !self.IsEnableSelectDirectionCondition() : !self.IsEnableSelectDirectionCondition());
          if (flag3)
            this.InternalLogUnitEnd();
    ...
    private void ExecuteSkill(ESkillTiming timing, LogSkill log, SkillData skill)
    {
      ...
      if (!this.IsBattleFlag(EBattleFlag.ComputeAI) && skill.IsPrevApply())
      {
        ...
        {
          Unit target = log.targets[index].target;
          this.BuffSkill(timing, self, target, skill, false, log, SkillEffectTargets.Target, false, (BuffEffect[]) null);
          target.CalcCurrentStatus(false, this.IsBattleFlag(EBattleFlag.PredictResult));
        }
        this.BuffSkill(timing, self, self, skill, false, log, SkillEffectTargets.Self, false, (BuffEffect[]) null);
        self.CalcCurrentStatus(false, this.IsBattleFlag(EBattleFlag.PredictResult));
        this.AbsorbAndGiveApply(self, skill, log);
      }
      ...
      else
      {
        if (skill.IsTransformSkill())
        {
          if (!this.IsBattleFlag(EBattleFlag.PredictResult) && log.targets != null && log.targets.Count > 0)
          {
            ...
            if ((int) self.CurrentStatus.param.hp != 0)
              self.KeepHp = self.CurrentStatus.param.hp;
            if (skill.EffectType == SkillEffectTypes.TransformUnitTakeOverHP && (int) self.KeepHp != 0)
              target.CurrentStatus.param.hp = self.KeepHp;
            self.ForceDead();
...
label_66:
              ...
              case SkillEffectTypes.GemsIncDec:
                int num3 = this.GetSkillEffectValue(self, target, skill, log);
...
label_71:
                ...
                if (skill.IsNormalAttack())
                {
                  ...
                  if (job != null && (job.ArtifactDatas != null || !string.IsNullOrEmpty(job.SelectedSkin)))
                  {
                    ...
                    for (int index2 = 0; index2 < artifactDataList.Count; ++index2)
                    {
                      ...
                      {
                        this.BuffSkill(timing, self, target, artifactData.BattleEffectSkill, false, log, SkillEffectTargets.Target, false, (BuffEffect[]) null);
          ...
          if (skill.IsNormalAttack())
          {
            ...
            if (job != null && (job.ArtifactDatas != null || !string.IsNullOrEmpty(job.SelectedSkin)))
            {
              ...
              for (int index = 0; index < artifactDataList.Count; ++index)
              {
                ...
                if (artifactData != null && artifactData.ArtifactParam != null && (artifactData.ArtifactParam.type == ArtifactTypes.Arms && artifactData.BattleEffectSkill != null) && artifactData.BattleEffectSkill.SkillParam != null)
                {
                  this.BuffSkill(timing, self, self, artifactData.BattleEffectSkill, false, log, SkillEffectTargets.Self, false, (BuffEffect[]) null);
        ...
        if (!this.IsBattleFlag(EBattleFlag.ComputeAI))
        {
          if (!this.IsBattleFlag(EBattleFlag.IsNotClearPrevBuff))
          {
            ...
            if (self.RemoveBuffPrevApply())
              self.CalcCurrentStatus(false, false);
          }
          if (!skill.IsPrevApply())
            this.AbsorbAndGiveApply(self, skill, log);
        }
        ...
        {
          for (int index = 0; index < log.targets.Count; ++index)
            this.AbilityChange(self, log.targets[index].target, skill);
          for (int index = 0; index < log.targets.Count; ++index)
            this.GridEventStart(self, log.targets[index].target, EEventTrigger.HpDownBorder, (SkillParam) null);
          for (int index = 0; index < log.targets.Count; ++index)
          {
            Unit target = log.targets[index].target;
            bool flag2 = false;
            ...
            if (target.IsDead)
            {
              ...
              this.Dead(self, target, DeadTypes.Damage, false);
            }
          ...
          using (List<Unit>.Enumerator enumerator = this.mUnits.GetEnumerator())
          {
            ...
            {
              Unit current = enumerator.Current;
              this.GridEventStart(self, current, EEventTrigger.WdUsedSkill, skill.SkillParam);
            }
          }
        }
        bool flag4 = false;
      ...
      for (int index1 = 0; index1 < defender.NotifyUniqueNames.Count; ++index1)
      {
        ...
        else if ((string) defender.NotifyUniqueNames[index1] == "nall")
        {
          using (List<Unit>.Enumerator enumerator = this.mUnits.GetEnumerator())
          {
            ...
            {
              Unit current = enumerator.Current;
              if (current.Side == EUnitSide.Neutral)
              {
                current.SetUnitFlag(EUnitFlag.DamagedActionStart, false);
                current.SetUnitFlag(EUnitFlag.Searched, true);
              }
            }
          }
        }
    ...
    public List<Unit> ContinueStart(long btlid, int seed)
    {
      ...
      using (List<Unit>.Enumerator enumerator = this.mPlayer.GetEnumerator())
      {
        ...
        {
          Unit current = enumerator.Current;
          if (!current.IsDead && (!current.IsNPC || current.IsEntry))
          {
            current.SetUnitFlag(EUnitFlag.EntryDead, true);
            current.ForceDead();
            unitList1.Add(current);
          }
        }
      }
    ...
    private void InvokeSkillBuffCond(Unit unit, SkillData skill, ESkillTiming timing)
    {
      ...
      if (effectRate > 0 && effectRate < 100 && (int) (this.GetRandom() % 100U) > effectRate)
        return;
      if (buffEffect1 != null)
        this.BuffSkill(timing, unit, unit, skill, false, (LogSkill) null, SkillEffectTargets.Target, false, (BuffEffect[]) null);
      if (buffEffect2 != null)
        this.BuffSkill(timing, unit, unit, skill, false, (LogSkill) null, SkillEffectTargets.Self, false, (BuffEffect[]) null);
      if (condEffect1 != null)
        this.CondSkill(timing, unit, unit, skill, false, (LogSkill) null, SkillEffectTargets.Target, true);
      if (condEffect2 == null)
        return;
      this.CondSkill(timing, unit, unit, skill, false, (LogSkill) null, SkillEffectTargets.Self, true);
    }
              ...
              using (List<SkillData>.Enumerator enumerator2 = current1.BattleSkills.GetEnumerator())
              {
                while (enumerator2.MoveNext())
                {
                  ...
                  if (current2.Timing == ESkillTiming.Dying && current2.Condition == ESkillCondition.Dying && (!current2.IsPassiveSkill() && this.CheckSkillCondition(current1, current2)))
                    this.InvokeSkillBuffCond(current1, current2, ESkillTiming.Dying);
                }
              }
            }
          }
        ...
        while (enumerator1.MoveNext())
        {
          ...
          if (!current1.IsDead)
          {
            using (List<SkillData>.Enumerator enumerator2 = current1.BattleSkills.GetEnumerator())
            {
              ...
              {
                SkillData current2 = enumerator2.Current;
                if (current2.Timing == ESkillTiming.JudgeHP && current2.Condition == ESkillCondition.JudgeHP && !current2.IsPassiveSkill())
                {
                  if (!this.CheckSkillCondition(current1, current2))
                    current1.RemoveJudgeHpLists(current2);
                  else if (!current1.IsContainsJudgeHpLists(current2))
                  {
                    this.InvokeSkillBuffCond(current1, current2, ESkillTiming.JudgeHP);
                    current1.AddJudgeHpLists(current2);
                  }
                }
              }
            }
    ...
    public IEnumerator UnitStartAsync()
    {
      ...
      return (IEnumerator) new BattleCore.\u003CUnitStartAsync\u003Ec__Iterator2A()
      {
        \u003C\u003Ef__this = this
      };
    }
              ...
              using (List<Unit>.Enumerator enumerator = this.mUnits.GetEnumerator())
              {
                while (enumerator.MoveNext())
                {
                  ...
                  if (!current.IsGimmick || current.IsBreakObj)
                  {
                    ...
                    {
                      if (!current.IsDead || !current.IsEntry || (current.IsSub || current.IsUnitFlag(EUnitFlag.UnitWithdraw)))
                        continue;
                    }
                    else if (current.IsDead || !current.IsEntry || current.IsSub)
                      continue;
                    if (this.CheckMatchUniqueName(current, eventTrigger.Tag))
                      unitList.Add(current);
                  }
                }
              }
    ...
    public bool IsOrdealValidNext()
    {
      ...
      using (List<Unit>.Enumerator enumerator = this.mPlayer.GetEnumerator())
      {
        ...
        {
          Unit current = enumerator.Current;
          if (current.TeamId == num && !current.IsDead)
            return true;
        }
      }
      return false;
    ...
    public void OrdealChangeNext()
    {
      ...
      {
        while (enumerator.MoveNext())
          enumerator.Current.ChargeTime = (OInt) 0;
      }
      using (List<Unit>.Enumerator enumerator = this.mPlayer.GetEnumerator())
      {
        while (enumerator.MoveNext())
        {
          Unit current = enumerator.Current;
          if (!current.IsUnitFlag(EUnitFlag.OtherTeam) && !current.IsDead)
          {
            current.SetUnitFlag(EUnitFlag.EntryDead, true);
            current.ForceDead();
          }
      ...
      using (List<Unit>.Enumerator enumerator = unitList.GetEnumerator())
      {
        ...
        {
          Unit current = enumerator.Current;
          current.CalcCurrentStatus(true, false);
          this.BeginBattlePassiveSkill(current);
        }
      }
    ...
    public void OrdealRestore(int team_id)
    {
      ...
      {
        while (enumerator.MoveNext())
          this.BeginBattlePassiveSkill(enumerator.Current);
      }
    }
...
label_64:
    ...
    private bool CalcMoveTargetAI(Unit self, bool forceAI)
    {
      ...
      for (int index1 = 0; index1 < list.Count; ++index1)
      {
        ...
        Grid[] gridArray = new Grid[4]
        {
          map[unitGridPosition.x - 1, unitGridPosition.y],
          map[unitGridPosition.x + 1, unitGridPosition.y],
          map[unitGridPosition.x, unitGridPosition.y + 1],
          map[unitGridPosition.x, unitGridPosition.y - 1]
        };
        int index2 = -1;
      ...
      if (num1 != 0)
      {
        ...
        if (defender != null)
        {
          ...
          if (skill.IsEnableHeightParamAdjust())
          {
            ...
            if (unitGridPosition1 != null && unitGridPosition2 != null)
            {
              int num10 = unitGridPosition1.height - unitGridPosition2.height;
              if (num10 > 0)
        ...
        int num11 = attackDetailType + num3 + num4 + num5 + num6 + num7 + num2 + num9;
        num1 += 100 * num1 * num11 / 10000;
      }
      return num1;
    }
    ...
    public class Json_BtlDrop
    {
      ...
      public EBattleRewardType dropItemType
      {
        get
        {
          ...
          string itype = this.itype;
          if (itype != null)
          {
            if (BattleCore.Json_BtlDrop.\u003C\u003Ef__switch\u0024map7 == null)
              BattleCore.Json_BtlDrop.\u003C\u003Ef__switch\u0024map7 = new Dictionary<string, int>(2)
              {
                {
                  "item",
                  0
            ...
            int num;
            if (BattleCore.Json_BtlDrop.\u003C\u003Ef__switch\u0024map7.TryGetValue(itype, out num))
            {
              if (num == 0)
