...
namespace SRPG
{
  public class MasterParam
  {
    ...
    private void AddUnitToItem()
    {
      ...
      using (Dictionary<string, UnitParam>.Enumerator enumerator = this.mUnitDictionary.GetEnumerator())
      {
        while (enumerator.MoveNext())
        {
          ...
          if (current.Value != null && !string.IsNullOrEmpty(current.Value.piece))
          {
            ItemParam itemParam = new ItemParam();
            itemParam.iname = current.Value.iname;
            if (this.mItemDictionary.ContainsKey(current.Value.piece))
            {
              ItemParam mItem = this.mItemDictionary[current.Value.piece];
              if (mItem != null)
              {
                itemParam.name = current.Value.name;
    ...
    public BuffEffectParam GetBuffEffectParam(string key)
    {
      ...
      using (List<BuffEffectParam>.Enumerator enumerator = this.mBuffEffectParam.GetEnumerator())
      {
        while (enumerator.MoveNext())
        {
          BuffEffectParam current = enumerator.Current;
    ...
    public CondEffectParam GetCondEffectParam(string key)
    {
      ...
      using (List<CondEffectParam>.Enumerator enumerator = this.mCondEffectParam.GetEnumerator())
      {
        while (enumerator.MoveNext())
        {
          CondEffectParam current = enumerator.Current;
          if (current.iname == key)
            return current;
        }
      }
    ...
    public List<SkillAbilityDeriveParam> FindAditionalSkillAbilityDeriveParam(SkillAbilityDeriveParam skillAbilityDeriveParam)
    {
      ...
      {
        using (List<SkillAbilityDeriveParam>.Enumerator enumerator = this.FindAditionalSkillAbilityDeriveParam(skillAbilityDeriveParam, deriveTrigger.m_TriggerType, deriveTrigger.m_TriggerIname).GetEnumerator())
        {
          while (enumerator.MoveNext())
          {
            SkillAbilityDeriveParam current = enumerator.Current;
    ...
    public List<SkillAbilityDeriveData> FindAllSkillAbilityDeriveDataWithArtifact(string artifactIname)
    {
      ...
      using (List<SkillAbilityDeriveData>.Enumerator enumerator = this.mSkillAbilityDerives.GetEnumerator())
      {
        while (enumerator.MoveNext())
        {
    ...
    public SkillAbilityDeriveData CreateSkillAbilityDeriveDataWithArtifacts(string[] artifactInames)
    {
      ...
      using (List<SkillAbilityDeriveTriggerParam[]>.Enumerator enumerator1 = combination.GetEnumerator())
      {
        while (enumerator1.MoveNext())
        {
          SkillAbilityDeriveTriggerParam[] current1 = enumerator1.Current;
          using (List<SkillAbilityDeriveData>.Enumerator enumerator2 = this.mSkillAbilityDerives.GetEnumerator())
          {
            while (enumerator2.MoveNext())
            {
              SkillAbilityDeriveData current2 = enumerator2.Current;
      ...
      if (abilityDeriveDataList.Count > 0)
      {
        ...
        using (HashSet<SkillAbilityDeriveParam>.Enumerator enumerator = abilityDeriveData2.m_AdditionalSkillAbilityDeriveParam.GetEnumerator())
        {
          while (enumerator.MoveNext())
          {
            SkillAbilityDeriveParam current = enumerator.Current;
        ...
        for (int index = 1; index < abilityDeriveDataList.Count; ++index)
        {
          ...
          using (HashSet<SkillAbilityDeriveParam>.Enumerator enumerator = abilityDeriveDataList[index].m_AdditionalSkillAbilityDeriveParam.GetEnumerator())
          {
            while (enumerator.MoveNext())
            {
              SkillAbilityDeriveParam current = enumerator.Current;
