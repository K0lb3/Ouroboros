...
namespace SRPG
{
  ...
  public class ShopBuySetConfirmWindow : MonoBehaviour, IFlowInterface
  {
    ...
    private void Refresh()
    {
      ...
      else
      {
        ...
        if (this.mShopitem.IsSet)
        {
          ...
          {
            GameObject gameObject = index >= this.shop_item_set_list.Count ? (GameObject) UnityEngine.Object.Instantiate<GameObject>((M0) this.ItemTemplate) : ((Component) this.shop_item_set_list[index]).get_gameObject();
    ...
    private void SetSliderValue(float newValue)
    {
      ...
      this.LimitedItemPriceText.set_text((ShopData.GetBuyPrice(this.mShopitem) * (int) this.AmountSlider.get_value()).ToString());
    }
