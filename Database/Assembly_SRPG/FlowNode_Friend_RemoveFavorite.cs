namespace SRPG
{
    ...
    [Pin(0, "Request", 0, 0), NodeType("System/Friend/RemoveFavorite", 0x7fe5), Pin(1, "Success", 1, 1)]
    public class FlowNode_Friend_RemoveFavorite : FlowNode_Network
    {
        ...
        [CompilerGenerated]
        private static void <OnActivate>m__1D4(GameObject go)
        {
        }

        [CompilerGenerated]
        private bool <OnSuccess>m__1D5(FriendData f)
        {
        ...
        public override void OnActivate(int pinID)
        {
            FriendData data;
            string str;
            <OnActivate>c__AnonStorey281 storey;
            if (pinID != null)
            {
                goto Label_0120;
            }
            storey = new <OnActivate>c__AnonStorey281();
            if (Network.Mode != 1)
            {
                goto Label_001E;
            }
        ...
        Label_0072:
            ...
            return;
        Label_0084:
            data = MonoSingleton<GameManager>.Instance.Player.Friends.Find(new Predicate<FriendData>(storey.<>m__1D3));
            if (data == null)
            {
                goto Label_00F0;
            }
            ...
            str = LocalizedText.Get("sys.FRIEND_NO_FAVORITE");
            if (<>f__am$cache1 != null)
            {
                goto Label_00E1;
            }
            <>f__am$cache1 = new UIUtility.DialogResultEvent(FlowNode_Friend_RemoveFavorite.<OnActivate>m__1D4);
        Label_00E1:
            UIUtility.SystemMessage(null, str, <>f__am$cache1, null, 0, -1);
            return;
        Label_00F0:
            this.mTargetFuid = storey.fuid;
            base.ExecRequest(new ReqFriendFavoriteRemove(storey.fuid, new Network.ResponseCallback(this.ResponseCallback)));
            base.set_enabled(1);
        Label_0120:
            return;
        }
        ...
        Label_0051:
            try
            {
                ...
                data = MonoSingleton<GameManager>.Instance.Player.Friends.Find(new Predicate<FriendData>(this.<OnSuccess>m__1D5));
                if (data == null)
                {
                    goto Label_0094;
                }
        ...
        [CompilerGenerated]
        private sealed class <OnActivate>c__AnonStorey281
        {
            internal string fuid;

            public <OnActivate>c__AnonStorey281()
            {
                base..ctor();
                return;
            }

            internal bool <>m__1D3(FriendData f)
            {
